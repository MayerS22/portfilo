...
const GlareCard = ({ 
  ...
  const glareOpacityTransform = useTransform(
    [mouseX, mouseY],
    ([x, y]: number[]) => Math.abs(x) + Math.abs(y) * glareOpacity
  )
  
  const glareTransform = useTransform(
    [mouseX, mouseY],
    ([x, y]: number[]) => `translateX(${x * 100}px) translateY(${y * 100}px) scale(${glareScale})`
  )
